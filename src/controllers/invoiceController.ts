...
// Crear factura con complemento de nómina por valores
export const createPayrollComplementByValues = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(500).json({ 
      succeeded: false, 
      message: 'Error al crear factura con complemento de nómina por valores', 
      error 
    });
  }
};

// Crear factura con complemento de impuestos locales por valores
export const createLocalTaxesComplementByValues = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(500).json({ 
      succeeded: false, 
      message: 'Error al crear factura con complemento de impuestos locales por valores', 
      error 
    });
  }
};
...
// Crear factura con complemento carta porte (lading complement)
export const createLadingComplement = async (req: Request, res: Response): Promise<void> => {
  try {
    const ladingInvoice: Invoice = {
      versionCode: "4.0",
      paymentFormCode: "01",
      paymentMethodCode: "PUE",
      currencyCode: "MXN",
      ...
      issuer: {
        ...
        taxCredentials: [
          {
            base64File: escuelaKemperbase64Cert,
            fileType: 0,
            password: password
          },
          {
            base64File: escuelaKemperBase64key,
            fileType: 1,
            password: password
          }
        ]
    ...
    const apiResponse = await fiscalapi.invoices.create(ladingInvoice);
    res.status(200).json(apiResponse);
  } catch (error) {
    res.status(500).json({ succeeded: false, message: 'Error al crear factura con complemento carta porte', error });
  }
};

// Crear factura con complemento carta porte (lading complement)
export const createLadingComplementByReference = async (req: Request, res: Response): Promise<void> => {
  try {
    const ladingInvoice: Invoice = {
      versionCode: "4.0",
      paymentFormCode: "01",
